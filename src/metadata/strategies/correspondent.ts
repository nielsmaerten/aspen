...
import { BasicCorrespondentResponseSchema, CorrespondentResponseSchema } from '../schemas.js';
  ...
  implements MetadataStrategy<'correspondent', EntitySelection, MetadataExtractionContext>
{
  ...
  async extract(
    ...
    const allowlistText = allowlist.length
      ? allowlist.map((item, index) => `${index + 1}. ${item.name}`).join('\n')
      : 'None';
    ...
    const response = await executeAiCall({
      ...
      wireSchema: BasicCorrespondentResponseSchema,
      context,
      messages,
    ...
    if (!candidate.name) {
      return {
        field: this.field,
        type: 'invalid' as const,
        message: 'Model did not provide a correspondent name',
      };
    }
